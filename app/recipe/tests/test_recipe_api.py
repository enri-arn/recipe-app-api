...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_filter_by_ingredient(self):
        ...
        r3 = create_recipe(user=self.user, title='Another title of recipe')
    ...
    def test_filter_recipe_by_title_contains(self):
        """Test filtering recipes by title contains."""
        r1 = create_recipe(user=self.user, title='Thai vegetable curry')
        create_recipe(user=self.user, title='Aubergine with Thaini')

        params = {'title_search': 'vegetable'}
        res1 = self.client.get(RECIPES_URL, params)

        s1 = RecipeSerializer(r1)

        self.assertEquals(len(res1.data), 1)
        self.assertEqual(res1.data[0], s1.data)

        params = {'title_search': 'bLe'}
        res2 = self.client.get(RECIPES_URL, params)

        s1 = RecipeSerializer(r1)

        self.assertEquals(len(res2.data), 1)
        self.assertEqual(res2.data[0], s1.data)
